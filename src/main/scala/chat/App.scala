...
class MessageSender(writer: EntityRef[ChatWriter.Command])(implicit system: ActorSystem[Nothing]) {
  ...
  def sendMessage(text: String): RIO[Env, Unit] = {
    ...
    def trySend(message: Message): RIO[Clock, Unit] = ZIO.fromFuture { _ =>
...
//      .retry(Schedule.spaced(5.second).untilOutput(_ > 100))
  ...
  def spamMessages(count: Int)(text: Int => String): RIO[Env, Unit] = {
    Stream.fromIterable(0 until count)
      .map(text)
      .foreach(sendMessage)
  }
...
case class App(system: ActorSystem[SystemCommand], runtime: Runtime[Clock]) {
  ...
  private def spamToChats(chats: Int, messages: Int)(text: Int => Int => String): RIO[Env, Unit] = {
    // https://github.com/zio/zio/issues/1499
    ZIO.foreachPar_((0 until chats).grouped(100).toVector) { ids =>
      ZIO.foreachPar_(ids) { chatId =>
  ...
  def startListeners(): Unit = {
    ...
    val res = ZIO.foreachPar_(0 until 8) { chatId =>
      ZIO.fromFuture(_ => system.ask[ActorRef[ChatListener.Command]](StartCharListener(s"Chat $chatId", _)))
    }
...
object App {
  ...
  case class StartCharListener(chatName: String, replyTo: ActorRef[ActorRef[ChatListener.Command]]) extends SystemCommand

  def behavior: Behavior[SystemCommand] = Behaviors.setup { ctx =>
